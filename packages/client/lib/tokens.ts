import { AsyncStorageBackends, KeygateOptions, StorageBackends } from "./main";
...
export interface SessionTokenKeeper<T extends StorageBackends> {
	...
	setSessionToken(token: string): T extends AsyncStorageBackends ? Promise<void> : void;
	sessionTokenStatus(): "expired" | "valid" | "missing";
}

export const createTokenKeeper = <T extends StorageBackends>(options: KeygateOptions<T>) => {
	...
	class SessionTokenKeeperImplementation implements SessionTokenKeeper<T> {
		constructor(options: KeygateOptions<T>) {
			if (!allowConstruct) throw new Error("SessionTokenKeeper can't be constructed");
		...
		beforeUnloadListener() {
			if (inMemorySessionToken) void this.setSessionTokenInStorage(inMemorySessionToken);
		}
		...
		setSessionToken(sessionToken: string): T extends "secureStorage" ? Promise<void> : void {
			inMemorySessionToken = sessionToken;
			return this.setSessionTokenInStorage(sessionToken) as unknown as T extends "secureStorage" ? Promise<void> : void; // is there a better way to do this in typescript?
		}

		setSessionTokenInStorage = (sessionToken: string) => storage?.setItem?.(sessionTokenKey, sessionToken);
...
export const selectStorageBackend = <T extends StorageBackends>(options: KeygateOptions<T>): Storage | undefined => {
	switch (options.storageBackend) {
		case "secureStorage": {
			if (!options.secureStorage) throw new Error("secureStorage is not provided");
			return options.secureStorage;
		}
		case "sessionStorage": {
			if (!isBrowser) throw new Error("sessionStorage is not supported in this environment");
